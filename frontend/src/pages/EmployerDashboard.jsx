...
const EmployerDashboard = () => {
  ...
  const handleApprove = async (id) => {
    const token = localStorage.getItem('authToken');
    await axios.post(`https://carbon-credits-backend.onrender.com/api/v1/employer/employees/${id}/approve`, {}, {
  ...
  const handleReject = async (id) => {
    const token = localStorage.getItem('authToken');
    await axios.post(`https://carbon-credits-backend.onrender.com/api/v1/employer/employees/${id}/reject`, {}, {
      headers: { Authorization: `Bearer ${token}` }
    });
    fetchData();
  };
